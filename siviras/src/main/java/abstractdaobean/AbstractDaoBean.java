...
public abstract class AbstractDaoBean<E> implements Dao<E> {
	...
	@PersistenceContext(unitName = "siviras")
	...
	public void remove(final E instance) {
		entityManager.remove(entityManager.contains(instance) ? instance : entityManager.merge(instance));
	}
	...
	public void remove(final E instance) {
		...
		E remove = instance;
		if (!contains) {
			remove = find(((AbstractEntity) instance).getId()); //?
		}
